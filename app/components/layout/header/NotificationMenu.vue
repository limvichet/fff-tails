...
<template>
  <div class="relative" ref="dropdownRef" v-if="hasRole('admin') || hasRole('ceo')">
    <!-- Notification Bell -->
    <button
    ...
    > -->
    <div v-if="dropdownOpen" class="absolute mt-2 flex w-[350px] max-h-[400px] flex-col rounded-2xl 
         border border-gray-200 dark:border-gray-700
         bg-white dark:bg-gray-900
         p-3 shadow-lg overflow-y-auto
         left-0 lg:right-0 lg:left-auto z-50">
      <!-- Header -->
      <h5 class="text-lg font-semibold mb-3 text-gray-800 dark:text-gray-200">
        Loan Notifications ({{ unreadCount }})
      </h5>

      <!-- List -->
      <ul class="divide-y divide-gray-200 dark:divide-gray-700">
        <li v-for="loan in notifications" :key="loan.id" @click="handleItemClick($event, loan)" :class="[
          'p-3 cursor-pointer transition rounded-lg',
          'text-gray-800 dark:text-gray-200',
          'hover:bg-gray-100 dark:hover:bg-gray-800',
          loan.loan_check_status === 0
            ? 'bg-blue-50 dark:bg-blue-900/20'
            : ''
        ]">
          <div class="flex justify-between items-center">
            <span>
              #{{ loan.id }} - {{ loan.loan_totalcash }}
            </span>

            <span v-if="loan.loan_check_status === 0" class="text-xs font-medium text-blue-600 dark:text-blue-400">
              Unread
            </span>
          </div>
        </li>
