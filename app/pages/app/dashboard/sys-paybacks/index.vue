...
definePageMeta({
  layout: "auth",
  requiresAuth: true,
  breadcrumb: { title: "Data Administration", subTitle: "Paybacks" },
  ssr: false,
})

useHead({
  title: "System Paybacks",
  meta: [{ name: "description", content: "System paybacks administration" }],
})
...
const fetchPaybacks = async () => {
  ...
  try {
    const res = await $fetch<ApiResponse>("/api/admin-secure/sys-paybacks", {
      method: "GET",
      query: {
        page: page.value,
    ...
    sysPaybacks.value = res.data.data ?? []
    total.value = res.data.total ?? 0
    lastPageValue.value = res.data.last_page ?? 1
  } catch (err: any) {
    errorMsg.value = err?.statusMessage || "Failed to fetch paybacks"
    sysPaybacks.value = []
  } finally {
    loading.value = false
  }
...
const validateForm = () => {
  ...
  if (!form.value.payback_kh?.trim()) {
    formErrors.value.payback_kh = "Required"
    valid = false
  }

  if (!form.value.payback_en?.trim()) {
    formErrors.value.payback_en = "Required"
    valid = false
  }
...
const savePayback = async () => {
  ...
  try {
    ...
    formData.append("payback_kh", form.value.payback_kh.trim())
    formData.append("payback_en", form.value.payback_en.trim())
    formData.append("active", String(form.value.active))

    if (isEditMode.value && selectedId.value) {
      formData.append("_method", "PUT")
      await $fetch(`/api/admin-secure/sys-paybacks/${selectedId.value}`, {
        method: "POST",
        body: formData,
      })
      showToast("Update successful", "Data updated successfully", "success")
    } else {
      await $fetch("/api/admin-secure/sys-paybacks", {
        method: "POST",
        body: formData,
      })
...
const openEditModal = (item: Payback) => {
  formErrors.value = {
    payback_kh: "",
    payback_en: "",
  }
  ...
  form.value = {
    payback_kh: item.payback_kh ?? "",
    payback_en: item.payback_en ?? "",
    active: Number(item.active),
  }
}

const resetForm = () => {
  form.value = { payback_kh: "", payback_en: "", active: 1 }
  formErrors.value = { payback_kh: "", payback_en: "" }
...
onMounted(fetchPaybacks)
</script>

<template>
  <div class="grid grid-cols-1">
    <ComponentCardPlus title="System Paybacks" :hasAdd="true" @add="isCreateModal = true">
        ...
        <div class="max-w-full overflow-x-auto custom-scrollbar">
          <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
              <tr v-for="(item, i) in sysPaybacks" :key="item.id"
                class="border-t border-gray-100 dark:border-gray-800 hover:bg-blue-300/20 transition">
                <td class="px-4 py-3 text-sm font-medium">
                  {{ (page - 1) * perPage + i + 1 }}
                </td>
                <td class="px-4 py-3 text-sm">{{ item.payback_kh }}</td>
                <td class="px-4 py-3 text-sm">{{ item.payback_en }}</td>
      ...
      <div v-if="isCreateModal"
        ...
        <div class="bg-white dark:bg-gray-900 w-full max-w-md p-6 rounded-xl shadow-xl">
          ...
          <div class="space-y-4">
            <div>
              <div class="flex items-center justify-between">
                ...
                <span v-if="formErrors.payback_kh" class="text-red-500 text-xs mt-1">{{ formErrors.payback_kh
                  }}</span>
              </div>
              <input v-model="form.payback_kh" placeholder="Enter text ..." class="input" />
            </div>

            <div>
              <div class="flex items-center justify-between">
                <label class="label">Title (EN) <span class="text-red-500 text-sm"> *</span></label>
                <span v-if="formErrors.payback_en" class="text-red-500 text-xs mt-1">{{ formErrors.payback_en
                  }}</span>
              </div>
              <input v-model="form.payback_en" placeholder="Enter text ..." class="input" />
            </div>
          ...
          <div class="flex justify-end gap-2 mt-6">
            ...
            <button @click="savePayback" :disabled="formLoading"
