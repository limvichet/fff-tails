...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const res = await $fetch<ApiResponse>(`${apiBaseUrl}/api/admin-secure/sys-sourcemoneys?page=${page}&param=${param}`, {
      method: "GET",
      headers: {
    ...
    const message = err?.data?.message || err?.message || "Failed to fetch sourcemoneys"
    throw createError({
