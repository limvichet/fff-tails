...
export default defineEventHandler(async (event: H3Event) => {
  ...
  try {
    ...
    const res = await $fetch<Data>(`${apiBaseUrl}/api/admin-secure/sys-identifications/${id}`, {
      method: "POST",
      body: formData,
      headers: {
    ...
    const message = err?.data?.message || err?.message || "Failed to update system identifications"
    throw createError({
